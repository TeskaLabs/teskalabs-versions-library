...
1. NGINX configuration will be changed. All requests will be redirected to `PUBLIC_URL` configured in the section `params` of the `/Site/model.yaml`.
    ```yaml
    params:
...
3. Correlators and Baseliners will be automatically deployed by the LMIO T-Rex microservice. Therefore, you need to remove all existing instances of the `lmio-correlator` and `lmio-baseliner` services from your `/Site/model.yaml` file.
