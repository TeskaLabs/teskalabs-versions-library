...
1. Correlators and Baseliners will be automatically deployed by the LMIO T-Rex microservice. Therefore, you need to remove all existing instances of the `lmio-correlator` and `lmio-baseliner` services from your `model.yaml` file.
...
2. Kibana index patterns (data views) will be automatically created and managed by LMIO Elman v25.30. Remove all manually created index patterns (data views) that start with the `lmio-` prefix to avoid conflicts.

    Open the Kibana UI, go to Stack Management > Index Patterns (or Data Views), and delete all index patterns (data views) that start with the `lmio-` prefix. Repeat this for every tenant.

3. Data Lifecycle Management policies for LMIO indices will be automatically created and managed by LMIO Elman v25.30.
...
1. Add the **LMIO Parser Builder** service definition to your `model.yaml` file:
    ...
    To enable the Parser Builder screen in the Web UI, add the web application to `/Site/ASAB Maestro/WebApps/LogMan.io WebUI.yaml`:
    ...
    mfe:
      https:
        # Add the following line
        /lmio_parser_builder_webui: lmio_parser_builder_webui
    ```

2. Add the **LMIO T-Rex** service definition to your `model.yaml` file:
    ...
    To enable the Detection screen in the Web UI, add the web application to `/Site/ASAB Maestro/WebApps/LogMan.io WebUI.yaml`:
    ...
    mfe:
      https:
        # Add the following line
        /lmio_detection_webui: lmio_detection_webui
    ```

3. Open the Detection screen in the Web UI. Select the Correlations and Detections you want to enable, and click the Apply button. Repeat this for every tenant.
...
4. Due to changes in Kafka Consumer group names for Correlators, a new Consumer group may be created when the Correlator starts. The old Consumer group is not automatically deleted. You can delete the old Consumer groups manually using Kafka management tools.
...
5. Enable **ZooKeeper monitoring** in Grafana dashboards. On every node running ZooKeeper, edit the `zoo.cfg` file located at `/data/hdd/zookeeper-<n>/conf/zoo.cfg`.
