...
(1) To accumulate values for scalar type attributes in one ticket, add an alert-specific type for this attribute in ECS Schema:
...
(2) An upgraded LMIO Alerts receive messages on every detection, regardless of their `trigger` section content. This increases the load significantly. Revise your active detection rules to receive only relevant security notifications.
