...
### ASAB Maestro v25.28.05

- lmio-webui - more resilient in case Schema is compromised

### ASAB Maestro v25.28.06

- lmio-webui - fixed the issue with the tenant in the params of `DELETE` request in Receiver's collector API endpoints
- asab-iris - added certificate bundle support for SMTP service

### ASAB Maestro v25.28.07

- mongo - MongoDB 7.0.28 contains a fix for CVE-2025-14847
...
- lmio-webui - fix timestamp condition for links in the  Events to Discover screen, issues with filtering incorrect values from Top values of table of Discover screen and `CountryIsoCodeRenderer`.
- lmio-common-library - updated description for `Windows Account Logon Failed`; `CheckPoint` content added to EventLane template
- lmio-elman - handling for missing tenant schema; compatibility with ASAB Library (when content is enabled, all parent nodes are enabled as well)
- lmio-collector & lmio-collector-system - an outdated collector replaced with the latest upgrade
- lmio-correlator & lmio-correlator-builder - solving the limitations of asab multiline configuration
- lmio-watcher - better error message for invalid geopoint
- lmio-franz - updated default configuration; updated grafana dashboards
- item `asab_maestro_library` in `define` section renamed to `asab_maestro_application` (versions compatibility note)

### Logman.io v25.28.06

- asab-maestro-library - added Grafana dashboards for monitoring LMIO Alerts & LMIO Franz dashboards replaced Burrow dashboards
- lmio-common-library - added an alerts-specific type for attribute `we.target.user.name`
- lmio-depositor - a lower threshold for events needed to initiate a rollover on indices
- lmio-elman - a fix for disabling files functionality
- lmio-watcher - binary files for removed lookups are also removed
- lmio-feeds - allowed headers in http inputs; added pagination
- lmio-ipaddrproc - fixed the issue that caused rebuilt of lookups before the library was initialized

### Logman.io v25.28.07

- lmio-watcher - removes items from cache during expiration
- lmio-parsec - updated splang
- lmio-alerts - returnded support for an obsolete range filter format (integrations issue)

### Logman.io v25.28.08

- lmio-parsec - updated splang
- lmio-franz - cleanup time changed
- lmio-collector - fix in SSL connection
...
2. Reconfigure Elasticsearch instances tier by tier, node by node.

    - Start with cold nodes. Reconfigure only one instance at the time. 
...
3. Once all instances are reconfigured, re-enable shard allocation.

    _The cluster should go from yellow to green._

    Open Kibana again, enter Dev Console and run the following request.
...
1. Update versions in `/Site/model.yaml`:
    ...
    applications:
    - name: "ASAB Maestro"
      version: v25.28  # ...replace with the highest available patch
    - name: "LogMan.io"
      version: v25.28  # ...replace with the highest available patch
    ```
...
2. Apply the changes to each node in the cluster one by one, consecutivelly.
    ...
    Note: if applying the changes does not work from WebUI, access the terminal. Run the following commands on each node:

    ```shell
    admin@node1:~$ /opt/site
    admin@node1:/opt/site$ ./gov.sh up
    ```
...
`lmio-baseliner*` and `lmio-correlator*` consumer groups were renamed to `lmio_baseliner*` and `lmio_correlator*` consumer groups. Kafka will remember the old names for several weeks and discard them eventually.
...
(1) To accumulate values for scalar type attributes in one ticket, add an alert-specific type for each attribute in ECS Schema:
...
(2) Upgraded LMIO Alerts receive messages on every detection now, regardless of the rules' `trigger` section content. This increases the load significantly. Revise your active detection rules to receive only relevant security notifications.
